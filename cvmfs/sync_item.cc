...
SyncItemType SyncItem::GetGenericFiletype(
    const SyncItem::EntryStat &stat) const {
...
SyncItemType SyncItem::GetRdOnlyFiletype() const {
  ...
  if (rdonly_stat_.error_code == ENOENT || rdonly_stat_.error_code == ENOTDIR)
    return kItemNew;
...
catalog::DirectoryEntryBase SyncItemNative::CreateBasicCatalogDirent(
  ...
  dirent.size_ = graft_size_ > -1 ? graft_size_ : this->GetUnionStat().st_size;
  ...
  if (this->IsSymlink()) {
    ...
    const ssize_t length = readlink((this->GetUnionPath()).c_str(), slnk,
                                    PATH_MAX);
    assert(length >= 0);
    ...
    if (info[0] == "size") {
      ...
      if (!String2Uint64Parse(info[1], &tmp_size)) {
        LogCvmfs(kLogFsTraversal, kLogWarning,
                 ...
                 "to integer: %s (errno=%d)",
                 trimmed_line.c_str(), strerror(errno), errno);
        continue;
      }
