...
CVMFS_DEBUGLOG=/tmp/cvmfs.${name}.log
EOF
...
setup_new_repository_mountpoints() {
  ...
  if [ x"$CVMFS_UNION_FS_TYPE" = x"overlayfs" ]; then
    echo -n "(overlayfs) "
    cat >> /etc/fstab << EOF
cvmfs2#$name $rdonly_dir fuse allow_other,fsname=$name,config=/etc/cvmfs/repositories.d/${name}/client.conf:${CVMFS_SPOOL_DIR}/client.local,cvmfs_suid,noauto,debug 0 0 # added by CernVM-FS for $name
