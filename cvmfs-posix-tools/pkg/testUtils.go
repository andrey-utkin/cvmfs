...
func ClearCvmfsCache() {
	cmd := exec.Command("rm", "-r", "-f", "/tmp/var/cache/cvmfs/test.repo")
...
func UmountRepo() {
	if !FullyContainerized() {
		cmd := exec.Command("fusermount", "-u", TestMount())
		cmd.Stdout = os.Stdout
		// cmd.Run()
		if err := cmd.Run(); err != nil {
			if err.Error() == "exit status 1" {
				fmt.Println("Nothing currently mounted, so nothing unmounted.")
			} else {
				panic(err)
			}
		}
...
func MountRepo() {
	if !FullyContainerized() {
		os.MkdirAll(TestMount(), 0750)
		cmd := exec.Command("cvmfs2", "-o", "config=../../pkg/etc/cvmfs/repositories.d/test.repo/client.conf",
			"test.repo", TestMount())
		cmd.Stdout = os.Stdout
		if err := cmd.Run(); err != nil {
			if err.Error() == "exit status 16" {
				fmt.Println("Resource was busy while setting up cvmfs repo, waiting and retrying...")
				cmdRetry := exec.Command("cvmfs2", "-o", "config=../../pkg/etc/cvmfs/repositories.d/test.repo/client.conf",
					"test.repo", TestMount())
