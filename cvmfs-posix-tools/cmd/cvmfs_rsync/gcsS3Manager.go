...
// Purge a single file from the bucket in s3Interface
func purgeSingleFile(ctx Context, pFile pkg.PurgeFile, s3Interface S3Interface, compressor *pkg.Compressor) ErrorData {
	if err := s3Interface.purge(pFile.PathStr); err != nil {
...
// Purge all of the files specified in db from the bucket in s3 interface
var purgeFiles = func(ctx Context, db pkg.DB, s3Interface, alternateS3Interface S3Interface) error {
	...
	s3Modifier[pkg.PurgeFile, ErrorData](ctx, filesPurged, s3Interface, alternateS3Interface, nil, purgeSingleFile, purgeErrManager)
...
func hashUpdateAndGetUploadReaders(ctx Context, workingUploadFile *pkg.UploadFile, fileErrorData *FileErrorData, srcInfo fs.FileInfo, hashReaders []pkg.NamedReader, f io.ReadSeekCloser, firstTime bool) ([]pkg.NamedReader, error) {
	uploadReaders := []pkg.NamedReader{}
	hashData, zeroLength, err := ctx.uploadHasher.HashFileFromReaderList(workingUploadFile.SrcPathString, hashReaders, ctx.cvmfsChunkSize, ctx.uploadCompressor)
	if err != nil {
		return nil, err
	}
...
func uploadSingleFile(ctx Context, uploadFile pkg.UploadFile, s3Interface S3Interface, compressor *pkg.Compressor) FileErrorData {
	...
	readers, f, err := pkg.GetFileReaders(workingUploadFile.SrcPathString, workingUploadFile.FileSize, ctx.cvmfsChunkSize, ctx.numUploadHashers)
	if err != nil {
	...
	if ctx.checksum {
		uploadReaders = []pkg.NamedReader{{Name: workingUploadFile.DestPathString, Reader: f, StartOffset: 0, PartSize: workingUploadFile.FileSize}}
		// TODO: ContentAddressable should be able to avoid re-hashing during checksumming as it will have the hashes present.
		// That being said, this is a relatively small use case (checksum over content addressable) so for the time being, will leave as is.
	} else {
		if uploadReaders, err = hashUpdateAndGetUploadReaders(ctx, &workingUploadFile, &fileErrorData, uploadFile.SrcInfo, readers, f, true); err != nil {
			fileErrorData.err = err
			return fileErrorData
		}
	}
	...
	for retries > 0 {
		...
		err := s3Interface.uploadReaderList(workingUploadFile.DestPathString, uploadReaders, partSize, ctx.numConcurrentUploaders, !ctx.cfg.Repo.DotScheme && !ctx.cfg.Repo.ContentAddressable, !ctx.cfg.Repo.NoHeadObjectCheck, &workingUploadFile, compressor)
		if err != nil {
			fileErrorData.err = err
			return fileErrorData
		}
		...
		if retryFileUpload(srcInfo, &workingUploadFile) {
			...
			readers, f, err = pkg.GetFileReaders(workingUploadFile.SrcPathString, srcInfo.Size(), ctx.cvmfsChunkSize, ctx.numUploadHashers)
			if err != nil {
				fileErrorData.err = err
				return fileErrorData
			}
			if uploadReaders, err = hashUpdateAndGetUploadReaders(ctx, &workingUploadFile, &fileErrorData, srcInfo, readers, f, false); err != nil {
				fileErrorData.err = err
				return fileErrorData
			}
			retries--
...
var uploadFiles = func(ctx Context, s3Interface, alternateS3Interface S3Interface, db pkg.DB) (UploadStatistics, error) {
	var span trace.Span
	ctx.trCtx, span = tr.Start(ctx.trCtx, "uploadFiles")
	defer span.End()
	...
	s3Modifier[pkg.UploadFile, FileErrorData](ctx, filesUploaded, s3Interface, alternateS3Interface, ctx.uploadCompressor, uploadSingleFile, fileErrorDataManager)
