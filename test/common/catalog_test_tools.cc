...
DirSpec::DirSpec() : items_(), dirs_() { dirs_.insert(""); }
...
bool DirSpec::AddFile(const std::string &name, const std::string &parent,
  ...
  const catalog::DirectoryEntry
      entry = catalog::DirectoryEntryTestFactory::RegularFile(name, size, hash);
...
bool DirSpec::LinkFile(const std::string &name, const std::string &parent,
                       ...
                       const XattrList &xattrs, shash::Suffix suffix) {
  const catalog::DirectoryEntry
      entry = catalog::DirectoryEntryTestFactory::Symlink(name, size, symlink);
  const std::string full_path = entry.GetFullPath(parent);
  items_.insert(std::make_pair(full_path, DirSpecItem(entry, xattrs, parent)));
  return true;
}
...
bool DirSpec::AddDirectory(const std::string &name, const std::string &parent,
  ...
  const catalog::DirectoryEntry
      entry = catalog::DirectoryEntryTestFactory::Directory(name, size);
  const std::string full_path = entry.GetFullPath(parent);
  items_.insert(
      std::make_pair(full_path, DirSpecItem(entry, XattrList(), parent)));
  return ret;
}
...
static void RemoveItemHelper(const DirSpec &spec,
    ...
    if (item.entry_base().IsDirectory()) {
      ...
      for (DirSpec::ItemList::const_iterator it = spec.items().begin();
        ...
        if (it->second.parent() == rel_full_path) {
          const std::string p = it->second.entry_base().GetFullPath(
              rel_full_path);
...
bool CatalogTestTool::Apply(const std::string &id, const DirSpec &spec) {
  ...
  for (DirSpec::ItemList::const_iterator it = spec.items().begin();
    ...
    if (item.entry_.IsRegular() || item.entry_.IsLink()) {
      ...
      catalog_mgr_->AddDirectory(item.entry_base(), item.xattrs(),
                                 item.parent());
    }
  }

  DirSpec::NestedCatalogList::const_iterator it;
  for (it = spec.nested_catalogs().begin(); it != spec.nested_catalogs().end();
       ++it) {
...
bool CatalogTestTool::LookupNestedCatalogHash(const shash::Any &root_hash,
  ...
  if (!(*nc_hash)) {
    LogCvmfs(kLogCatalog, kLogStderr,
             "nested catalog for directory '%s' cannot be found", path.c_str());
    return false;
  }
...
bool CatalogTestTool::FindEntry(const shash::Any &root_hash,
                                const std::string &path,
                                catalog::DirectoryEntry *entry) {
  perf::Statistics stats;
  UniquePtr<catalog::WritableCatalogManager> catalog_mgr(
...
catalog::WritableCatalogManager *CatalogTestTool::CreateCatalogMgr(
  ...
  catalog::WritableCatalogManager
      *catalog_mgr = new catalog::WritableCatalogManager(
          root_hash, stratum0, temp_dir, spooler, dl_mgr, false, 0, 0, 0, stats,
          false, 0, 0, "");
  catalog_mgr->Init();
...
void CatalogTestTool::CreateHistory(string repo_path_,
...
void CatalogTestTool::CreateManifest(string repo_path_,
  ...
  unsigned char *sig;
  unsigned sig_size;
  ASSERT_TRUE(signature_mgr->Sign(
...
void CatalogTestTool::CreateKeys(string repo_path_,
                                 string *public_key,
                                 shash::Any *hash_cert) {
  ...
  ASSERT_TRUE(
      SafeWriteToFile(masterkey, repo_path_ + "/testrepo.masterkey", 0600));
  ASSERT_TRUE(SafeWriteToFile(certificate, repo_path_ + "/testrepo.crt", 0600));
