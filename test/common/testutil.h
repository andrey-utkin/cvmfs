 ...
 public:
  ...
  static DerivedT *MockConstruct() {
    PolymorphicConstructionUnittestAdapter::RegisterPlugin<
        upload::AbstractUploader, DerivedT>();
...
template<class DerivedT>
const std::string
    AbstractMockUploader<DerivedT>::sandbox_tmp_dir = g_sandbox_tmp_dir;
...
class MockObjectStorage {
 ...
 public:
  ...
  static void UnregisterObjects() {
    typename MockObjectStorage<ObjectT>::AvailableObjects::const_iterator i,
        iend;
    ...
    typename MockObjectStorage<ObjectT>::AvailablePathObjects::const_iterator j,
        jend;
  ...
  static ObjectT *Get(const shash::Any &hash) {
    return (Exists(hash) && !IsDeleted(hash)) ? available_objects[hash] : NULL;
  }
...
class MockCatalog : public MockObjectStorage<MockCatalog> {
 ...
 public:
  ...
  struct File {
    ...
    File()
        ...
        , name("")
        , is_nested_catalog_mountpoint(false) { }
    ...
    catalog::DirectoryEntry ToDirectoryEntry() const {
      ...
      if (is_directory)
        return catalog::DirectoryEntryTestFactory::Directory(
            name, size, hash, is_nested_catalog_mountpoint);
 ...
 public:
  ...
  MockCatalog(const MockCatalog &other)
      ...
      , catalog_hash_(other.catalog_hash_)
      , catalog_size_(other.catalog_size_)
      , revision_(other.revision_)
      , last_modified_(other.last_modified_)
      , is_root_(other.is_root_)
      , owns_database_file_(false)
      , active_children_(other.active_children_)
 ...
 protected:
  MockCatalog *Clone() const { return new MockCatalog(*this); }
...
class MockHistory : public history::History,
 ...
 public:
  ...
  MockHistory(const bool writable, const std::string &fqrn);
 ...
 private:
  ...
  static const shash::Any &get_hash(const Tag &tag) { return tag.root_hash; }
  ...
  struct TagRemover {
    ...
    bool operator()(const Tag &tag) { return history_->Remove(tag.name); }
