...
void T_Ingestion::ExerciseCompressionRoundtrip(zip::Algorithm alg) {
  ...
  do {
    ...
    if (b->size() > 0) {
      ...
      EXPECT_EQ(0, memcmp(ptr_read_decomp, block_raw.data(), decomp_read_pos));
    }
    if (alg == zip::kNoCompression) {
      ASSERT_EQ(read_pos, decomp_read_pos);
      EXPECT_EQ(0, memcmp(ptr_read_large, ptr_read_decomp, read_pos));
    }
  ...
  if (alg == zip::kNoCompression) {
    ...
    EXPECT_EQ(0, memcmp(ptr_read_large, block_raw.data(), block_raw.size()));
  }
